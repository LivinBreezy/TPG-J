...
public class APIExecutionExample2 
{
	public static void main(String[] args)
	{
		...
		long[] actions = new long[dataLength];
			...
			for( int j=0; j < gamesToPlay; j++ )
			{
				...
				while( tpg.remainingTeams() > 0 )
				{
					...
					while( count > 0 )
					{
						...
						int outcome = (int)tpg.participate( inputFeatures );

						//System.out.println("Outcome " + outcome + " versus " + input[count-1][outcome]);
						
						if( input[count-1][outcome] == 1.0 )
							reward += 1.0;
